...
const DiscountManagement = () => {
    ...
    useEffect(() => {
        if (availableMenuItems.length > 0) {
            ...
            // console.log(menuItemFilter, ' jkl ', menuItemFilter.length);
    ...
    // useEffect(() => {
    //     console.log(menuItemFilter, ' jkl ', menuItemFilter.length);
    //     console.log('Filter changed to: ', menuItemFilter, menuItemFilter.length);
    //   }, [menuItemFilter]);      
    
    // Handle menu item selection
    const handleMenuItemSelect = (item) => {
        // Check if item is already selected
        const isSelected = selectedMenuItems.some(selected => selected.dish_id === item.dish_id);
        
        if (isSelected) {
            // Remove item if already selected
            setSelectedMenuItems(prev => prev.filter(selected => selected.dish_id !== item.dish_id));
            // console.log('i', selectedMenuItems);
        } else {
    ...
    const handleCreateCoupon = async (e) => {
        ...
        const validUntil = new Date(formatDate(couponValidUntil));
        const today = new Date();
        if(validUntil < today){
            alert('Coupon Expiry date should be before today')
            return;
        }

        // Discount Value should be less than Minimum Order Value
        const discountValue = parseInt(couponValue);
        if(minOrderInt <= discountValue){
            alert('Discount value should be less than minimum order value')
            return;
        }
        ...
        <div className="discount-management-container">
            ...
            <div className="top-nav">
                ...
                <div className="logo-container">
                    <Link to="/canteen-manager-home" style={{display: 'flex', alignItems: 'center', textDecoration: 'none'}}>
                                        ...
                                        <div className="menu-items-list">
                                            {filteredMenuItems.length > 0 ? (
                                                filteredMenuItems.map(item => (
                                                    <div
                                                        key={item.id}
                                                        className={`menu-item ${selectedMenuItems.some(selected => selected.id === item.id) ? '' : ''}`}
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            handleMenuItemSelect(item);
                                                        }}
                                                    >
                                                        <div className="item-details">
                                                            <span className="item-name">{item.name || item.dish_name} </span>
