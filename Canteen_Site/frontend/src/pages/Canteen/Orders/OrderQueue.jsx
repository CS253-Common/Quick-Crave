...
const OrderQueue = () => {
    ...
    const mapOrderStatus = (status) => {
        ...
        switch (numStatus) {
            ...
            case 2:
                ...
                break;
            case 3:
                statusString = 'Cooking';
                break;
            case 4:
                statusString = 'Ready for Pickup/Delivery';
                break;
            default:
                statusString = 'Unknown';
        }
        
        console.log(`Status code ${status} mapped to "${statusString}"`);
        return statusString;
    };
    ...
    const handleCompleteOrder = async (order_id) => {
        try {
            setLoading(true);
            const result = await canteenService.completeOrder(order_id);
    ...
    const filteredOrders = orders.filter(order => {
        const matchesSearch = searchQuery === '' || 
            ...
            (Array.isArray(order.dishes) && order.dishes.some(dish => 
                dish.dish_name && dish.dish_name.toLowerCase().includes(searchQuery.toLowerCase())
            ));

        // Make sure the status comparison is exact
        const matchesStatus = filterValue === 'All Orders' || order.status === filterValue;
        
        // Debug log to check why filtering might not work
        if (filterValue !== 'All Orders' && order.status !== filterValue) {
            console.log(`Order ${order.order_id} filtered out: filter value "${filterValue}" != order status "${order.status}"`);
        }
                                                ...
                                                <span>
                                                    ...
                                                    {Array.isArray(order.dishes) && order.dishes.length > 0 
                                                        ? order.dishes.map((dish, index) => 
                                                            `${dish.quantity || 1} x ${dish.dish_name || 'Unknown Item'}`
                                                          ).join(', ')
                                                        : "No items"
                                                    }                                                
                                                </span>
