    ...
    const fetchReservations = async () => {
        try {
            ...
            const formattedReservations = data.map(item => {
                ...
                let statusCode = 1; // Default to pending (1)
                if (item.status === 'approved') statusCode = 2;
                else if (item.status === 'rejected') statusCode = 0;
                else if (item.status === 'completed') statusCode = 3;
                ...
                return {
                    ...
                    status: statusCode, // Use numeric status code
                                                ...
                                                {reservation.status === 1 ? (
                                                    ...
                                                    <span className={`status-badge status-${reservation.status}`}>
                                                        {reservation.status === 0 ? 'Rejected' : 
                                                         ...
                                                         reservation.status === 2 ? 'Approved' : 'Completed'}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
