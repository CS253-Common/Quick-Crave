...
const ReservationManagement = () => {
    ...
    // Map for Reservation status
    const orderReservation = new Map([
        [0, "Rejected"],
        ...
        const fetchReservations = async () => {
            try {
                ...
                const formattedReservations = data.map(item => ({
                    ...
                    date: formatDate(item.date_of_reservation),
                    ...
                    status: item.status,
                    image: item.customer_image_url || 'https://randomuser.me/api/portraits/lego/1.jpg'
                }));
                ...
                console.log('Swa', formattedReservations)
                
                // Default filter to show only pending reservations
                const pendingReservations = formattedReservations.filter(item => item.status === 1);
                ...
                // const approvedCount = data.filter(item => item.status === 3).length;
                // const rejectedCount = data.filter(item => item.status === 0).length;

                let rejectedCount = 0;
                let pendingCount = 0;
                let approvedCount = 0;

                formattedReservations.forEach(({ status }) => {
                    if (status == 0) rejectedCount++;
                    else if (status == 1) pendingCount++;
                    else if (status == 3) approvedCount++;
                });
    ...
    const handleAccept = async (id) => {
        try {
            ...
            await canteenService.acceptReservation(id);
    ...
    const handleReject = async (id) => {
        try {
            ...
            // Make API call to update reservation status
            await canteenService.rejectReservation(id);
                                            ...
                                            <div className="action-buttons">
                                                {reservation.status === 'pending' ? (
                                                    ...
                                                    <span className={`status-badge ${reservation.status}`}>
                                                        {orderReservation.get(reservation.status)}
