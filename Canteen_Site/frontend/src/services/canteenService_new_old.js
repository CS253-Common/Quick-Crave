...
const mockOrders = [
    ...
    },
    {
        ...
        dishes: [
            { dish_name: "Pizza", quantity: 1, price: 15.50 },
            { dish_name: "Soda", quantity: 2, price: 2.50 }
        ],
    ...
    },
    {
        ...
        dishes: [
            ...
            { dish_name: "Garlic Bread", quantity: 1, price: 7.00 },
            { dish_name: "Salad", quantity: 1, price: 10.00 }
        ],
    ...
    },
    {
        ...
        dishes: [
            { dish_name: "Noodles", quantity: 1, price: 14.50 },
            { dish_name: "Spring Rolls", quantity: 2, price: 4.00 }
        ],
    ...
    },
    {
        ...
        dishes: [
            { dish_name: "Biryani", quantity: 1, price: 25.00 },
            { dish_name: "Raita", quantity: 1, price: 5.00 }
        ],
...
const mockRecentOrders = [
    ...
    },
    {
        ...
        dishes: [
            { dish_name: 'Masala Dosa', quantity: 2 }
        ],
        total_amount: 160,
...
const validateCoupon = (data) => {
    if (!data.code || !data.value || !data.min_order_value || !data.usage_limit || !data.valid_until) {
        throw new Error('Missing required fields for coupon');
    }

    if (data.code.length > 12) {
        throw new Error('Coupon code must be 12 characters or less');
    }

    const value = parseFloat(data.value);
    if (isNaN(value) || value <= 0 || value > 100) {
        throw new Error('Coupon value must be a valid percentage between 1 and 100');
    }

    const minOrderValue = parseInt(data.min_order_value);
    if (!Number.isInteger(minOrderValue) || minOrderValue <= 0) {
        throw new Error('Minimum order value must be a positive integer');
    }
...
const canteenService = {
  ...
  markOrderReady: async (order_id) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/canteen/orders/ready`,
        { order_id },
        {
          headers: { 'Content-Type': 'application/json' },
          timeout: 10000
  ...
  completeOrder: async (order_id) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/canteen/orders/complete`,
        { order_id },
        {
          headers: { 'Content-Type': 'application/json' },
          timeout: 10000
      ...
      if (response.status === 200) {
        return { success: true, message: response.data.message || 'Order ready for pickup/delivery' };
      } else {
  ...
  rejectOrder: async (order_id) => {
    try {
      ...
      if (response.status === 200) {
        return { success: true, message: response.data.message || 'Order rejected successfully' };
      } else {
  ...
  getOrderDetails: async (order_id) => {
    try {
      const response = await axios.get(`${API_URL}/canteen/orders/${order_id}`, {
        timeout: 10000
      });
  ...
  deleteDiscount: async (discountId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        return { success: true, message: response.data.message || 'Discount Deleted' };
      } else {
  ...
  deleteCoupon: async (couponId) => {
    try {
      ...
      console.log(`Coupon ${couponId} deleted successfully`);
        return { success: true, message: response.data.message || 'Coupon Deleted' };
      } else {
  ...
  createDiscount: async (discountData) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/canteen/create-discounts`, {
      ...
      console.log('Discount created successfully:', response.data);
        return { success: true, message: response.data.message || 'Discounts Updated Successfully' };
      } else {
  ...
  createCoupon: async (couponData) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/canteen/create-coupon`, {
      ...
      console.log('Coupon created successfully:', response.data);
        return { success: true, message: response.data.message || 'Coupon Created Successfully' };
      } else {
  ...
  addMenuItem: async (menuItemData) => {
    try {
      ...
      const apiData = {
        ...
        // status: menuItemData.status | image-url
      };
      ...
      if (menuItemData.imageFile) {
        ...
        const response = await axios.post(`${API_URL}/canteen/add-dish`, formData, {
          headers: { 
        ...
        const response = await axios.post(`${API_URL}/canteen/add-dish`, apiData, {
          headers: { 
  ...
  updateMenuItem: async (itemId, menuItemData) => {
    try {
      ...
      if (menuItemData.imageFile) {
        ...
        const response = await axios.post(`${API_URL}/canteen/edit-dish/${itemId}`, formData, {
          headers: { 
            'Content-Type': 'multipart/form-data',
            'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}`
          },
        ...
        const response = await axios.post(`${API_URL}/canteen/edit-dish/${itemId}`, apiData, {
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}`
          },
  ...
  deleteMenuItem: async (itemId) => {
    try {
      const response = await axios.post(`${API_URL}/canteen/menu-items/delete/${itemId}`, {}, {
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}`
        },
  ...
  changePassword: async (passwordData) => {
    try {
        const response = await axios.post(`${API_URL}/canteen/change-password`, {
  ...
  updateCanteenProfile: async (profileData) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/canteen/profile`, apiProfileData, {
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}`
        },
  ...
  updateProfileImage: async (imageFile) => {
    try {
        ...
        const response = await axios.post(`${API_URL}/canteen/profile-image`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
                'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}`
            }
  ...
  getReservations: async (canteenId) => {
    try {
      ...
      console.log('Reservations fetched successfully:', response.data.reservations);
  ...
  getPopularOrders: async () => {
    try {
      const response = await axios.post(`${API_URL}/canteen/orders/popular`, {}, {
        headers: { 'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}` }
      });
      return response.data;
    } catch (error) {
  ...
  getRecentOrders: async () => {
    try {
      const response = await axios.post(`${API_URL}/canteen/orders/recent`, {}, {
        headers: { 'Authorization': `Bearer ${sessionStorage.getItem('auth_token')}` }
      });
      return response.data;
    } catch (error) {
  ...
  getHomeDiscountData: async () => {
    try {
        ...
        const response = await axios.post(`${API_URL}/canteen/home-discount-n-coupons`)
        const data = response.data;
        console.log(data);
        return {
  ...
  getHomeReservationData: async () => {
    try {
        const response = await axios.post(`${API_URL}/canteen/home-reservation-queue`);
  ...
  getTrendingPicks: async () => {
    try {
        ...
        return {
            ...
            items: [
                { id: 1, name: 'Tandoori Chicken', image_url: 'https://images.unsplash.com/photo-1603360946369-dc9bb6258143?ixlib=rb-1.2.1&auto=format&fit=crop&w=300&q=80', orders: 45 },
                { id: 2, name: 'French Fries', image_url: 'https://images.unsplash.com/photo-1630384060421-cb20d0e0649d?ixlib=rb-1.2.1&auto=format&fit=crop&w=300&q=80', orders: 38 },
                { id: 3, name: 'Burger', image_url: 'https://images.unsplash.com/photo-1551183053-bf91a1d81141?ixlib=rb-1.2.1&auto=format&fit=crop&w=300&q=80', orders: 32 },
                { id: 4, name: 'Peanut Butter Sandwich', image_url: 'https://images.unsplash.com/photo-1588315029754-2dd089d39a1a?ixlib=rb-1.2.1&auto=format&fit=crop&w=300&q=80', orders: 30 },
                { id: 5, name: 'Biryani', image_url: 'https://images.unsplash.com/photo-1589302168068-964664d93dc0?ixlib=rb-1.2.1&auto=format&fit=crop&w=300&q=80', orders: 28 }
            ]
        };
    }
  },
