...
const userData = {
  ...
  const saveBooking = async (bookingData) => {
    ...
    try{
    // Prepare the data to match your backend API requirements
    // const {canteen_id, num_people, reservation_time, additional_requests, customer_id } = req.body;

    const reservationData = {
      canteen_id: canteen.id, // or get from canteenData if needed
      date: bookingData.date,
      time: bookingData.time,
      num_people : bookingData.people,
      additional_requests: bookingData.specialRequest
    };

    // Make the API call
    const response = await axios.post(
      'http://localhost:4000/customer/customer-add-reservations', 
      reservationData
    );
    ...
    // You might want to store the reservation ID from the backend response
    setReservationId(response.data.reservation_id || bookingData.bookingId);
