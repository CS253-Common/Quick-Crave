...
const JWT_EXPIRES_IN = '5m'; // 5 minutes for testing
...
exports.login = async (req, res) => {
  ...
  const generateTokenAndSetCookie = (customer_id, userType, res) => {
      const token = jwt.sign({ customer_id, userType }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
...
exports.addUser = async (req,res) => {
    ...
    const { userType, name, username, phone, email, password } = req.body;
    ...
    const enterUser = await db.query(`INSERT INTO customers (username, password, name, phone_number, email, wallet_balance, cart_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING customer_id`, [username, password, name, phone, email, 0, 0]);
    console.log('added to database');
    return res.status(200).json({});
}

exports.signup = async (req, res) => {
    const { userType, name, username, phone, email, password, confirmPassword } = req.body;
    ...
    try {
        if (userType === 'customer') {
            ...
            }else return res.status(200).json({ message: 'OTP sent. Proceed to OTP verification.', otp: hashed_otp });
        } else if (userType === 'canteen-manager') {
...
exports.resetPassword = async (req, res) => {
    const { userType, username, new_password } = req.body;
    const userIdField = `${userType}_id`;
    const table = `${userType}s`;

    try {
        await db.query(`UPDATE ${table} SET password = $1 WHERE username = $2`, [new_password, username]);
